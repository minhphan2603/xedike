...
router.post("/register", (req, res) => {
  ...
  if (!isValid) {
    ...
    User.findOne({ $or: [{ email }, { phone }] })
      ...
      .catch(err => res.status(400).json({ email: err }));
  }
});
  ...
  User.findOne({ email })
    .then(user => {
      ...
      bcrypt.compare(password, user.password, (err, isMatched) => {
        ...
        jwt.sign(payload, fingerprint, { expiresIn: "1h" }, (err, token) => {
          if (err) {
...
router.get(
  "/",
  authenticating,
  authorizing(["passenger", "driver"]),
  (req, res) => {
    res.status(200).json("ban da qua duoc Authorization");
  }
);
